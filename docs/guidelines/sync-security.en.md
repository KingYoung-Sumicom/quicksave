...
- The blob payload is `SyncPayloadV3` (`apps/pwa/src/lib/syncMerge.ts`); every synced field is wrapped in `Timestamped<T>` and the machine list pairs with `machineTombstones`. Synced scalars currently include `masterSecret`, `apiKey` (Anthropic), and `voiceConfig` (the serialized Whisper transcription key/baseUrl/model — one opaque LWW string so voice input is configured once and revoked/rotated in one place). New optional fields stay backward-compatible: older blobs lacking them merge as `null`. **`cachedProjects` is deliberately stripped from the wire** (`buildLocalPayload` sends `{}`): it is agent-declared data (the agent is the source of truth via `project:list-summaries`), kept as a local-only per-device cache and refreshed on connect, so it never bloats the blob. `applySyncedState` preserves each device's local copy across a sync pull; `mergeSyncPayloads`/`syncPayloadsEqual` ignore it.
