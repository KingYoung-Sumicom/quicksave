...
├── terminal/
│   └── terminalManager.ts    # PTY pool + headless xterm state per terminal
├── files/
...
> **Terminal subsystem**: `TerminalManager` (above) is a standalone EventEmitter that does not share state with AI sessions. It uses `node-pty` to open a shell (default `$SHELL -l`) and mirrors each PTY in a headless xterm with 1,000 scrollback rows. `/terminals/:id/output` snapshots serialize that screen as replayable VT state, including a bounded line history; live updates remain raw PTY chunks. The PWA subscribes to `/terminals` and `/terminals/:id/output`, then replays each output snapshot at its captured dimensions before applying later chunks. Terminal commands reject while disconnected so old input or create requests do not run after reconnect.
...
buffering streaming ASR final fragments until an intent endpoint is reached,
...
| `/terminals/:terminalId/output` | `TerminalOutputSnapshot \| null` (serialized VT screen + bounded scrollback + seq + size + exit status) | `TerminalOutputChunk` (next chunk of output, monotonic `seq`) | async `terminalManager.outputSnapshot()` + PTY `'data'` event |
...
**The `__repoPath` smuggle for repo-scoped commands**: the bus protocol has no envelope-level metadata, so `useGitOperations.sendCommand` stuffs the current repoPath into the reserved `__repoPath` field on `git:*` and commit-summary payloads. The adapter pulls it out and puts it back into `msg.repoPath`; `messageHandler` rejects repo-scoped requests that omit it or point at an unavailable repo. Responses mirror the server-acknowledged repoPath back to `data.__repoPath` so the PWA can validate scope.
...
| `opencode:` | Machine-local OpenCode management. `config-snapshot` is a sanitized summary of OpenCode version/schema plus MCP, provider/model, agent, skill/command, plugin, Exa, and Guardian status (including the daemon-wide reviewer server reachability state); API keys are never returned. `mcp-upsert` and `mcp-remove` persist global MCP configuration on the paired machine. `websearch-update` persists the per-agent Exa opt-in and restarts only its OpenCode child. `guardian-update` persists the Guardian model-server URL/model, write-only API key, timeout, and manual-escalation threshold in `~/.quicksave/agent.json` (and invalidates the daemon-wide server-reachability state); `QUICKSAVE_GUARDIAN_*` environment variables take precedence and make the UI read-only. `guardian-test` runs a 1-token probe against the stored or draft endpoint and only records daemon-wide server state when the probed config IS the active one. |
