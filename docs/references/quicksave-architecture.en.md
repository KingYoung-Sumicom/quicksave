...
`service/run.ts` calls `wireLegacyBusVerbs(bus, messageHandler)` at startup. Every request-response verb in the `LEGACY_BUS_VERBS` array (`git:*`, `ai:*`, `agent:*`, `claude:*`, `session:*`, `project:*`, `push:*`, `codex:*`, `terminal:*`, `files:list`, `files:read`, plus `ping`) is registered as `bus.onCommand(verb, ...)`. The adapter wraps the payload back into a `Message` envelope, dispatches it to the existing `messageHandler.handleMessage`, then translates the result back into a resolved payload or a rejected Error. Structured errors are encoded as `"CODE: message"` strings. `files:rtc-*` is wired directly by `wireFileRtcStream`, like streaming voice, because it publishes trickled ICE asynchronously.
