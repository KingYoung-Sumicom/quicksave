...
├── files/
│   └── fileBrowser.ts        # 唯讀檔案瀏覽器（list / read，pair 後允許任意 host path）
└── git/
...
> **File browser 子系統**：`FileBrowser`（`apps/agent/src/files/fileBrowser.ts`）是純 request-response、無狀態的唯讀模組——沒有 EventEmitter、沒有 bus subscription，因為檔案內容是 on-demand 抓取而非串流。Pair 完成後它刻意不做 root confinement：absolute `path` 會照原樣讀，relative path 以 `realpath(cwd)` 為基準 resolve，但不 assert 仍在 root 內。二進位偵測用前 8 KiB 的 NUL byte sniff，文字預覽預設上限 1 MiB（`maxBytes` 可覆寫，但被 hard-clamp 在 4 MiB）；inline image read 另有 16 MiB 上限。
...
所有 request-response verb（`git:*`、`ai:*`、`agent:*`、`claude:*`、`session:*`、`project:*`、`push:*`、`codex:*`、`terminal:*`、`files:*`）在啟動時被註冊成 `bus.onCommand(verb, ...)`。Adapter 把 payload 包回 `Message` 信封，dispatch 給既有 `messageHandler.handleMessage`，把結果翻譯回 resolved payload 或 reject Error。結構化錯誤編碼成 `"CODE: message"` 字串（例如 `MISSING_REPO_PATH` 或 `REPO_NOT_AVAILABLE`）。
...
**Repo-scoped commands 的 `__repoPath` smuggle**：bus 協議沒有信封級 metadata，所以 `useGitOperations.sendCommand` 把當前 repoPath 塞到 `git:*` 與 commit-summary payload 的保留欄 `__repoPath`；adapter 讀出後放回 `msg.repoPath`。`messageHandler` 會拒絕未帶 repoPath 或指向不可用 repo 的 repo-scoped request，回應時再把 server 認可的 repoPath 鏡射回 data 的 `__repoPath` 讓 PWA 做 scope 驗證。
...
- `push:notify` 的 `extra` 會放穩定 JSON array，覆蓋 `sessionId`、`title`、`body`、`agentId`、`url`、`tag`；任何可見通知欄位被竄改都會使簽章失效。
...
兩種觸發產生同 `{title, body, sessionId, tag, agentId}` 結構；`tag: sessionId` 讓後續訊息在瀏覽器端自動摺疊成單一通知。OS 層通知文字刻意保持泛用（`Quicksave`、需要權限/輸入、session 已就緒），不得包含 prompt、transcript、command string 或 tool input。Service Worker 只會開啟 same-origin app URL；外部 URL 會退回 `/`。
...
**PWA XSS threat model**：正常 agent/model output 進到 card 時不應被視為 trusted HTML。這條路徑要變成可利用的 UI XSS，通常需要模型或工具先輸出 payload，還要剛好碰到 renderer / sanitizer / URL policy 漏洞，讓 payload 變成瀏覽器可執行內容。它屬於 high impact、lower likelihood 的 hardening/regression 風險，優先度低於 agent 權限、pairing、filesystem、notification 與 relay signature 邊界。維持 raw HTML disabled；未來改 markdown、syntax highlighting、preview 或 `dangerouslySetInnerHTML` 時都要當成 security review point。
