...
export function buildClaudeCliArgs(opts: {
  ...
  if (opts.bypassFlagPath) {
    ...
    // AskUserQuestion, ExitPlanMode, and ExitWorktree are intentionally excluded —
    ...
    const conditionalCommand = `input=$(cat); [ -f "${escapedPath}" ] && ! printf '%s' "$input" | grep -qE '"tool_name":"(AskUserQuestion|ExitPlanMode|ExitWorktree)"' && printf '{"hookSpecificOutput":{"hookEventName":"PermissionRequest","decision":{"behavior":"allow"}}}' || true`;
