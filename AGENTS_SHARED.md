...
- When restarting the daemon from inside a daemon-spawned agent session, use `scripts/dev-daemon-delayed.sh` with enough delay for the current tool call to return. The delayed restart is forced by default and daemon-owned provider sessions will be killed; there is no non-disruptive restart path without adding a separate wrapper. Preserve its explicit `bash scripts/dev-daemon.sh` invocation because `scripts/dev-daemon.sh` is tracked without an executable bit.
